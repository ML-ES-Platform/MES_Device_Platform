...
void ed_can_mcp_obd_setup();
void ed_can_mcp_obd_loop();
...
// CAN RX Variables
unsigned long ed_can_mcp_obd_rx_Id;
uint8_t dlc;
uint8_t rxBuf[8];

// CAN Interrupt and Chip Select
#define ED_CAN_MCP_OBD_CAN_0_INT 2                              // Set CAN0 INT to pin 2
MCP_CAN ED_CAN_MCP_OBD_CAN_0(9);                                // Set CAN0 CS to pin 9
...
void ed_can_mcp_obd_setup()
{
  ...
  if(ED_CAN_MCP_OBD_CAN_0.begin(MCP_STDEXT, CAN_500KBPS, MCP_20MHZ) == CAN_OK)
...
#if standard == 1
  // Standard ID Filters
  ED_CAN_MCP_OBD_CAN_0.init_Mask(0,0x7F00000);                // Init first mask...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(0,0x7DF0000);                // Init first filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(1,0x7E10000);                // Init second filter...
  
  ED_CAN_MCP_OBD_CAN_0.init_Mask(1,0x7F00000);                // Init second mask... 
  ED_CAN_MCP_OBD_CAN_0.init_Filt(2,0x7DF0000);                // Init third filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(3,0x7E10000);                // Init fourth filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(4,0x7DF0000);                // Init fifth filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(5,0x7E10000);                // Init sixth filter...

#else
  // Extended ID Filters
  ED_CAN_MCP_OBD_CAN_0.init_Mask(0,0x90FFFF00);                // Init first mask...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(0,0x90DB3300);                // Init first filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(1,0x90DA0100);                // Init second filter...
  
  ED_CAN_MCP_OBD_CAN_0.init_Mask(1,0x90FFFF00);                // Init second mask... 
  ED_CAN_MCP_OBD_CAN_0.init_Filt(2,0x90DB3300);                // Init third filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(3,0x90DA0100);                // Init fourth filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(4,0x90DB3300);                // Init fifth filter...
  ED_CAN_MCP_OBD_CAN_0.init_Filt(5,0x90DA0100);                // Init sixth filter...
#endif
  
  ED_CAN_MCP_OBD_CAN_0.setMode(MCP_NORMAL);                          // Set operation mode to normal so the MCP2515 sends acks to received data.

  pinMode(ED_CAN_MCP_OBD_CAN_0_INT, INPUT);                          // Configuring pin for /INT input
...
void ed_can_mcp_obd_loop()
{
  if(!digitalRead(ED_CAN_MCP_OBD_CAN_0_INT))                         // If CAN0_INT pin is low, read receive buffer
  {
    ED_CAN_MCP_OBD_CAN_0.readMsgBuf(&ed_can_mcp_obd_rx_Id, &dlc, rxBuf);             // Get CAN data
    
    // First request from most adapters...
    if(ed_can_mcp_obd_rx_Id == FUNCTIONAL_ID){
      obdReq(rxBuf);
    }       
  }
}
...
void obdReq(uint8_t *data){
  uint8_t numofUint8_ts = data[0];
  uint8_t mode = data[1] & 0x0F;
  uint8_t pid = data[2];
  bool tx = false;
  uint8_t txData[] = {0x00,(0x40 | mode),pid,PAD,PAD,PAD,PAD,PAD};
...
//    }
  ...
  //=============================================================================
      ...
      uint8_t DTCs[] = {(0x40 | mode), 0x05, 0xC0, 0xBA, 0x00, 0x11, 0x80, 0x13, 0x90, 0x45, 0xA0, 0x31};
...
//    }
      ...
      uint8_t VIN[] = {(0x40 | mode), pid, 0x01, 0x31, 0x5a, 0x56, 0x42, 0x50, 0x38, 0x41, 0x4d, 0x37, 0x44, 0x35, 0x32, 0x32, 0x30, 0x31, 0x38, 0x31};
...
//    }
      ...
      uint8_t CID[] = {(0x40 | mode), pid, 0x01, 0x41, 0x72, 0x64, 0x75, 0x69, 0x6E, 0x6F, 0x20, 0x4F, 0x42, 0x44, 0x49, 0x49, 0x73, 0x69, 0x6D, 0x51, 0x52, 0x53, 0x54};
...
//    }
      ...
      uint8_t CVN[] = {(0x40 | mode), pid, 0x02, 0x11, 0x42, 0x42, 0x42, 0x22, 0x43, 0x43, 0x43};
...
//    }
      ...
      uint8_t ECMname[] = {(0x40 | mode), pid, 0x01, 0x45, 0x43, 0x4D, 0x00, 0x2D, 0x41, 0x72, 0x64, 0x75, 0x69, 0x6E, 0x6F, 0x4F, 0x42, 0x44, 0x49, 0x49, 0x73, 0x69, 0x6D};
...
//    }
      ...
      uint8_t ESN[] = {(0x40 | mode), pid, 0x01, 0x41, 0x72, 0x64, 0x75, 0x69, 0x6E, 0x6F, 0x2D, 0x4F, 0x42, 0x44, 0x49, 0x49, 0x73, 0x69, 0x6D, 0x00};
...
//  }
  ...
  if(tx)
    ED_CAN_MCP_OBD_CAN_0.sendMsgBuf(REPLY_ID, 8, txData);
}
...
//    packetcnt++;
  ...
  ED_CAN_MCP_OBD_CAN_0.sendMsgBuf(REPLY_ID, 8, tpData);
  ...
  while(not_done){
    ...
    if(!digitalRead(ED_CAN_MCP_OBD_CAN_0_INT)){
      ED_CAN_MCP_OBD_CAN_0.readMsgBuf(&ed_can_mcp_obd_rx_Id, &dlc, rxBuf);
    
      if((ed_can_mcp_obd_rx_Id == LISTEN_ID) && ((rxBuf[0] & 0xF0) == 0x30)){
