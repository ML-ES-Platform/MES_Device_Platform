...
float srv_sns_air_press_cond(float press_raw)
{
  ...
  srv_sig_cond_fifo_push_float(press_raw, srv_sns_air_press_buff_in, INPUT_BUFF_SIZE);
  // print_buff(press_buff_in, INPUT_BUFF_SIZE);

  // 1.2. luam o copie din buferul de intrare
  srv_sig_cond_buff_copy_float(srv_sns_air_press_buff_in, srv_sns_air_press_buff_med, MEDIAN_BUFF_SIZE);
  // print_buff(press_buff_med, MEDIAN_BUFF_SIZE);

  // 1.3. sortam copia
  srv_sig_cond_buff_sort_float(srv_sns_air_press_buff_med, MEDIAN_BUFF_SIZE);
  // print_buff(press_buff_med, MEDIAN_BUFF_SIZE);
  ...
  srv_sig_cond_fifo_push_float(press_median, srv_sns_air_press_buff_lpf, LPF_BUFF_SIZE);
  // print_buff(press_buff_lpf, LPF_BUFF_SIZE);

  // 2.2 evaluam media ponderata
  float press_flt = srv_sig_cond_buff_wavg_float(srv_sns_air_press_buff_lpf, srv_sns_air_press_weights_lpf, LPF_BUFF_SIZE);
