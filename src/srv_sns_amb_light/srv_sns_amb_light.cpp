...
float srv_sns_amb_light_cond(float temp_raw)
{
  ...
  srv_sig_cond_fifo_push_float(temp_raw, srv_sns_amb_light_buff_in, INPUT_BUFF_SIZE);
  // print_buff(temp_buff_in, INPUT_BUFF_SIZE);

  // 1.2. luam o copie din buferul de intrare
  srv_sig_cond_buff_copy_float(srv_sns_amb_light_buff_in, srv_sns_amb_light_buff_med, MEDIAN_BUFF_SIZE);
  // print_buff(temp_buff_med, MEDIAN_BUFF_SIZE);

  // 1.3. sortam copia
  srv_sig_cond_buff_sort_float(srv_sns_amb_light_buff_med, MEDIAN_BUFF_SIZE);
  // print_buff(temp_buff_med, MEDIAN_BUFF_SIZE);
  ...
  srv_sig_cond_fifo_push_float(temp_median, srv_sns_amb_light_buff_lpf, LPF_BUFF_SIZE);
  // print_buff(temp_buff_lpf, LPF_BUFF_SIZE);

  // 2.2 evaluam media ponderata
  float temp_flt = srv_sig_cond_buff_wavg_float(srv_sns_amb_light_buff_lpf, srv_sns_amb_light_weights_lpf, LPF_BUFF_SIZE);
