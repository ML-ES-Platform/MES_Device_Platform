...
void srv_ui_serial_task(void *pvParameters);
void test_module_task(void *pvParameters);
...
//-----------------------------------------------------------------------------
#ifdef USE_ED_BH1750
    xTaskCreate(ed_bh1750_task, "ed_bh1750_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
#ifdef USE_ED_BMP
...
//-----------------------------------------------------------------------------
#ifdef USE_ED_DHT
    xTaskCreate(ed_dht_task, "ed_dht_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    xTaskCreate(srv_com_mqtt_task, "srv_com_mqtt_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    xTaskCreate(srv_heartbeat_task, "srv_heartbeat_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    xTaskCreate(srv_ui_serial_task, "srv_ui_serial_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
#ifdef USE_TEST_MODULE
    xTaskCreate(test_module_task, "test_module_task", 1024, NULL, 1, NULL);
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Task for managing APP
#ifdef USE_APP
void app_task(void *pvParameters)
{
    app_setup();
    vTaskDelay(APP_OFFSET / portTICK_PERIOD_MS);
    TickType_t xLastWakeTime = xTaskGetTickCount();

    for (;;)
    {
        app_loop();
        vTaskDelayUntil(&xLastWakeTime, APP_REC / portTICK_PERIOD_MS);
    }
}
#endif

//-----------------------------------------------------------------------------
// Task for managing CTRL_AIR_HUM
#ifdef USE_CTRL_AIR_HUM
void ctrl_air_hum_task(void *pvParameters)
{
    ctrl_air_hum_setup();
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_AIR_PRESS
#ifdef USE_CTRL_AIR_PRESS
void ctrl_air_press_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_AIR_TEMP_HEAT
#ifdef USE_CTRL_AIR_TEMP_HEAT
void ctrl_air_temp_heat_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_AIR_TEMP_VENT
#ifdef USE_CTRL_AIR_TEMP_VENT
void ctrl_air_temp_vent_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_AMB_LIGHT
#ifdef USE_CTRL_AMB_LIGHT
void ctrl_amb_light_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_CAN_STEER
#ifdef USE_CTRL_CAN_STEER
void ctrl_can_steer_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_SERVO_ADC_DC
#ifdef USE_CTRL_SERVO_ADC_DC
void ctrl_servo_adc_dc_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_SOIL_MOIST
#ifdef USE_CTRL_SOIL_MOIST
void ctrl_soil_moist_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_WHEEL_STEER
#ifdef USE_CTRL_WHEEL_STEER
void ctrl_wheel_steer_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing CTRL_WHEEL_TRACT
#ifdef USE_CTRL_WHEEL_TRACT
void ctrl_wheel_tract_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing DD_WATER_PUMP
#ifdef USE_DD_WATER_PUMP
void dd_water_pump_task(void *pvParameters)
{
    dd_water_pump_setup();
...
//-----------------------------------------------------------------------------
// Task for managing ED_SNS_SOIL_MOIST
#ifdef USE_ED_SNS_SOIL_MOIST
void ed_sns_soil_moist_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_COM_MQTT
#ifdef USE_SRV_COM_MQTT
void srv_com_mqtt_task(void *pvParameters)
{
    srv_com_mqtt_setup();
...
//-----------------------------------------------------------------------------
// Task for managing SRV_HEARTBEAT
#ifdef USE_SRV_HEARTBEAT
void srv_heartbeat_task(void *pvParameters)
{
    srv_heartbeat_setup();
...
//-----------------------------------------------------------------------------
// Task for managing SRV_SNS_AIR_HUM
#ifdef USE_SRV_SNS_AIR_HUM
void srv_sns_air_hum_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_SNS_AIR_PRESS
#ifdef USE_SRV_SNS_AIR_PRESS
void srv_sns_air_press_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_SNS_AIR_TEMP
#ifdef USE_SRV_SNS_AIR_TEMP
void srv_sns_air_temp_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_SNS_AMB_LIGHT
#ifdef USE_SRV_SNS_AMB_LIGHT
void srv_sns_amb_light_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_SNS_SOIL_MOIST
#ifdef USE_SRV_SNS_SOIL_MOIST
void srv_sns_soil_moist_task(void *pvParameters)
{
...
//-----------------------------------------------------------------------------
// Task for managing SRV_UI_SERIAL
#ifdef USE_SRV_UI_SERIAL
void srv_ui_serial_task(void *pvParameters)
{
    srv_ui_serial_setup();
    vTaskDelay(SRV_UI_SERIAL_OFFSET / portTICK_PERIOD_MS);
    TickType_t xLastWakeTime = xTaskGetTickCount();

    for (;;)
    {
        srv_ui_serial_loop();
        vTaskDelayUntil(&xLastWakeTime, SRV_UI_SERIAL_REC / portTICK_PERIOD_MS);
    }
}
#endif

//-----------------------------------------------------------------------------
// Task for managing TEST_MODULE
#ifdef USE_TEST_MODULE
void test_module_task(void *pvParameters)
{
    test_module_setup();
    vTaskDelay(TEST_MODULE_OFFSET / portTICK_PERIOD_MS);
    TickType_t xLastWakeTime = xTaskGetTickCount();

    for (;;)
    {
        test_module_loop();
        vTaskDelayUntil(&xLastWakeTime, TEST_MODULE_REC / portTICK_PERIOD_MS);
    }
}
#endif
