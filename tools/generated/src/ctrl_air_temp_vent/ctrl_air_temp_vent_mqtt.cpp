...
void ctrl_air_temp_vent_mqtt_publish(DynamicJsonDocument &doc_out, PubSubClient &mqttClient)
{
  ...
  doc_out["cur_hum"] = ctrl_air_temp_vent_get_current_hum();
...
int ctrl_air_temp_vent_mqtt_callback(DynamicJsonDocument &doc_in, char *topic, byte *payload, unsigned int length)
{
  ...
  if (strcmp(topic, ctrl_air_temp_vent_mqtt_subscribe_topic) == 0)
  {
    ...
    {
      int out = value_f;
      if (out == CTRL_AIR_TEMP_VENT_OUT_OFF)
      {
        dd_valve_off(DD_VALVE_CTRL_AIR_TEMP_VENT_ID);
      }
      else if (out == CTRL_AIR_TEMP_VENT_OUT_ON)
      {
        dd_valve_on(DD_VALVE_CTRL_AIR_TEMP_VENT_ID, CTRL_AIR_TEMP_VENT_OP_D_TIME);
      }
    }
  }
  else
